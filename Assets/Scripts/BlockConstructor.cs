...
public class BlockConstructor : MonoBehaviour {
    ...
    public void SpawnBlock() {
        transform.localScale = new Vector3(Random.Range(0.5f, 3f), 1, 1);
    ...
    private static void ConstructBlockTriggers(Transform newBlock, params Transform[] triggerTransforms) {
        foreach (var triggerTransform in triggerTransforms) {
            triggerTransform.localScale = new Vector3(0.04f * (7 / newBlock.localScale.x), 0.1f, 1);
        }
    }

    private void ConstructBlock(GameObject newBlock) {
        newBlock.transform.localScale = new Vector3(Random.Range(0.3f, 7f), 6, 1);
